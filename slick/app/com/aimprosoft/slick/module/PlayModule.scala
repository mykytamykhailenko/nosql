...
class PlayModule extends AbstractModule {
  override def configure(): Unit = {
    bind(new TypeLiteral[BasicDAO[DBIO, Employee]] {}).toInstance(SlickActionLang(employeeTable))
    bind(new TypeLiteral[BasicDAO[DBIO, Department]] {}).toInstance(SlickActionLang(departmentTable))
